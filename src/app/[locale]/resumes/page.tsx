...
const MiniResumePreview = ({ resume }: { resume: ResumeData }) => (
  <div className="h-full w-full space-y-1.5 overflow-hidden bg-white p-3 text-[6px] leading-tight">
    ...
    <div className="border-b border-slate-100 pb-1.5 text-center">
      ...
      <div className="truncate text-slate-500">{resume.title || "Professional Title"}</div>
    </div>

    {/* Summary placeholder */}
    {resume.summary && (
      <div className="space-y-0.5">
        <div className="text-[7px] font-bold text-slate-600 uppercase">Summary</div>
        <div className="line-clamp-2 text-slate-400">{resume.summary.replace(/<[^>]*>/g, "").substring(0, 100)}</div>
      </div>
    )}
    ...
    {resume.skills?.length > 0 && (
      <div className="space-y-0.5">
        <div className="text-[7px] font-bold text-slate-600 uppercase">Skills</div>
        <div className="flex flex-wrap gap-0.5">
          {resume.skills.slice(0, 5).map((skill, i) => (
            <span key={i} className="rounded bg-blue-50 px-1 py-0.5 text-[5px] text-blue-700">
              {skill}
            </span>
          ))}
          {resume.skills.length > 5 && <span className="text-[5px] text-slate-400">+{resume.skills.length - 5} more</span>}
        </div>
      </div>
    )}
    ...
    <div className="space-y-0.5 pt-1">
      <div className="text-[7px] font-bold text-slate-600 uppercase">Experience</div>
